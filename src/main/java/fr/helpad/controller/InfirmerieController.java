...
public class InfirmerieController {
	...
	public String saveMedic(@ModelAttribute Medicament medicament, HttpServletRequest request,
		...
		if (nom.length() > 2 && nom.length() < 30)
			...
			redirectAttributes.addFlashAttribute("message", "Erreur : Le nom du médicament doit contenir entre 3 et 30 caractères.");
		...
		if (fonction.length() > 2 && fonction.length() < 40)
			...
			redirectAttributes.addFlashAttribute("message", "Erreur : La fonction du médicament doit contenir entre 3 et 40 caractères.");
		    redirectAttributes.addFlashAttribute("alertClass", "alert alert-danger alert-dismissible fade show");
			return "redirect:/ajouter-medicament";
		}
		...
		if (stock >= 0 && stock <= 999)
			...
			redirectAttributes.addFlashAttribute("message", "Erreur : La quantité en stock doit être un nombre positif entre 0 et 999");
		    redirectAttributes.addFlashAttribute("alertClass", "alert alert-danger alert-dismissible fade show");
			return "redirect:/ajouter-medicament";
		}
		...
		try {
			...
			redirectAttributes.addFlashAttribute("message", "Erreur : Le format de la quantité par boîte est invalide.");
		    redirectAttributes.addFlashAttribute("alertClass", "alert alert-danger alert-dismissible fade show");
			return "redirect:/ajouter-medicament";
		}
		if (intParBoite > 0 && intParBoite <= 10000)
			...
			redirectAttributes.addFlashAttribute("message", "Erreur : Quantité par boîte incorrecte, elle doit être comprise entre 1 et 10000");
		    redirectAttributes.addFlashAttribute("alertClass", "alert alert-danger alert-dismissible fade show");
			return "redirect:/ajouter-medicament";
		}
